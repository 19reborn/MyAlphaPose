...
def write_json(all_results, outputpath, form=None, for_eval=False, view = None, hand_face = False):
    '''
    ...
    for im_res in all_results:
        ...
        for human in im_res['result']:
            ...
            if form == 'cmu': # the form of CMU-Pose
            ...
            elif form == 'open': # the form of OpenPose  body_25
                if result['image_id'] not in json_results_cmu.keys():
                    json_results_cmu[result['image_id']]={}
                    json_results_cmu[result['image_id']]['version']="AlphaPose v0.3"
                ...
                result['keypoints'].append((result['keypoints'][16]+result['keypoints'][19])/2)
                result['keypoints'].append((result['keypoints'][17]+result['keypoints'][20])/2)
                indexarr=[0,78,18,24,30,15,21,27,57,36,42,48,33,39,45,6,3,12,9,60,66,72,63,69,75]
                for i in indexarr:
                ...
                #     indexarr = 
                #     for i in indexarr:
                #         tmp['face_keypoints_2d']
                #     for i in indexarr:
                #         tmp['hand_left_keypoints_2d'].
                #     for i in indexarr:
                #         tmp['hand_right_keypoints_2d']
    ...
    if form == 'cmu': # the form of CMU-Pose
        with open(os.path.join(outputpath,'alphapose-results.json'), 'w') as json_file:
            ...
            for name in json_results_cmu.keys():
                with open(os.path.join(outputpath,view,name.split('.')[0]+'_keypoints.json'),'w') as json_file:
                    json_file.write(json.dumps(json_results_cmu[name]))
    elif form == 'open': # the form of OpenPose
        #with open(os.path.join(outputpath,'alphapose-results.json'), 'w') as json_file:
            #json_file.write(json.dumps(json_results_cmu))
        if not os.path.exists(os.path.join(outputpath,view)):
            os.mkdir(os.path.join(outputpath,view))
        for name in json_results_cmu.keys():
            with open(os.path.join(outputpath,view,name.split('.')[0]+'_keypoints.json'),'w') as json_file:
                json_file.write(json.dumps(json_results_cmu[name]))
    else:
